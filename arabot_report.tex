...
\textbf{Description [5 marks]:} The project's dataset (\texttt{arabic\_words.py}) contains 200 words meticulously grouped into 20 semantic categories (e.g., "Family", "Food", "Technology", "Emotions"). The quizzes are structured to present words within these themes, limiting the session size to prevent cognitive overload and maximize short-term memory encoding. \\
...
\begin{itemize}[label={--}]
    \item \textbf{By group [3 marks]:} The system generates a "Vocabulary Mastery by Category" heatmap (bar chart) displaying accuracy percentages and total attempts broken down by the 20 thematic categories.
